...
class Estimator(models.Model):
    ...
    hours_perfect = fields.Float(
        store=True, compute="total_task_calc", string="Perfect (hours)"
    )
    hours_real_time = fields.Float(
        store=True, compute="calc_peal_time", string="Real Time (hours)"
    )
    hours_low_performance = fields.Float(
        store=True, compute="calc_ow_performance", string="Low performance (hours)"
    )
    notes = fields.Text(string="Description")
    unit_works_lines = fields.One2many(
        "task_estimation.lines", "task_id", string="Tasks Lines"
    )
    name_task_seq = fields.Char(
        string="Task Estimation Reference",
        required=True,
        copy=False,
        readonly=True,
        index=True,
    ...
    tasks_count = fields.Integer(
        string="Count of author tasks", compute="get_count_tasks"
    )
    task_id = fields.Many2one("estimator.project")
    role = fields.Many2one(
        "estimator.command_roles",
        string="Role",
    ...
    def _compute_author_role(self):
        for rec in self:
            write_data = rec.env["estimator.command"].search(
                [("id", "=", rec.author.id)]
            )
    ...
    def create(self, vals):
        if vals.get("name_task_seq", _("New")) == _("New"):
            vals["name_task_seq"] = self.env["ir.sequence"].next_by_code(
                "estimator.task_estimation.sequence"
            ) or _("New")
    ...
    def calc_peal_time(self):
        self.hours_real_time = (
            self.hours_perfect
            * float(self.technical_risks)
            * float(self.comprehension_index)
        )
    ...
    def get_count_tasks(self):
        count = self.env["estimator.task_estimation"].search_count(
...
class TaskEstimationLines(models.Model):
    ...
    workunit_id = fields.Many2one(
        "task_estimation.work_units", string="Work Unit ID", ondelete="cascade"
    )
    quantity = fields.Integer(string="Quantity")
    minutes_to_do = fields.Float(
        string="Time to perform", related="workunit_id.minutes_to_do"
    )
...
class Project(models.Model):
    ...
    total_perfect_hours = fields.Float(
        string="Total Perfect Hours", compute="total_calc_per_hours"
    )
    total_perfect_hours_by_role = fields.Float(
        "Total Perfect Hours by Role", compute="_compute_total_hours_by_role"
    )
    total_real_time = fields.Float(
        string="Total Real Time", compute="total_calc_real_time"
    )
    total_real_time_by_role = fields.Float(
        string="Total Real Time by Role", compute="_compute_total_hours_by_role"
    )
    total_low_performance = fields.Float(
        string="Total Low Performance", compute="total_calc_low_performance"
    )
    total_low_performance_by_role = fields.Float(
        string="Total Low Performance by Role", compute="_compute_total_hours_by_role"
    )
    tasks = fields.One2many(
        "estimator.task_estimation", "task_id", string="List of Tasks"
    )
    ...
    def _compute_total_hours_by_role(self):
        records = self.env["estimator.task_estimation"].search(
            [("role", "=", self.role_id.id), ("project_id", "=", self.name.id)]
        )
        for rec in records:
    ...
    def total_calc_per_hours(self):
        for task in self:
            ...
            if task.project_risk:
                task.total_perfect_hours = task.total_perfect_hours + (
                    task.total_perfect_hours * task.project_risk / 100
                )
    ...
    def total_calc_real_time(self):
        for task in self:
            ...
            if task.project_risk:
                task.total_real_time = task.total_real_time + (
                    task.total_real_time * task.project_risk / 100
                )
    ...
    def total_calc_low_performance(self):
        for task in self:
            ...
            if task.project_risk:
                task.total_low_performance = task.total_low_performance + (
                    task.total_low_performance * task.project_risk / 100
                )
        ...
        if task_id:
            return {
                ...
                "view_id": self.env.ref(
                    "estimator.task_estimation_project_form", False
                ).id,
                "res_model": "estimator.task_estimation",
                "target": "new",
            }
